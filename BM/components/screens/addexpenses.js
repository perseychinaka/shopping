...
export const AddExpenses = ({ navigation, route }) => {
  ...
  const saveExpense = async () => {
    ...
    if (client) {
      let data = { function: 'addOperation', data: { name, ucost, date, type: 'Expense',id:await uuid.v4(), user: currentuser.name },clientip,severip };
      if (!locked) { Lock(true); client.write(JSON.stringify(data)); }
    }
    else DB.addOperation(name, ucost, date, 'Expense', currentuser.name).then(r => {
      if ((typeof (r)).toString().toLowerCase() == 'number') {
        navigation.pop();
        ToastAndroid.showWithGravity(name + '  expense saved successfully',
        ToastAndroid.SHORT,
        ToastAndroid.CENTER);
      }
      else 
      ToastAndroid.showWithGravity(name + "there has been an error saving data",
      ToastAndroid.SHORT,
      ToastAndroid.CENTER);
      ToastAndroid("there has been an error saving data");
    });
    navigation.goBack();
